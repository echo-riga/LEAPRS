...
export async function createPpmpAction(data: PpmpPayload) {
...
export async function updatePpmpAction(data: PpmpPayload) {
  const session = await auth.api.getSession({ headers: await headers() });
  if (!session || session.user.role !== "admin") throw new Error("Forbidden");
...
// School Years
export async function createSchoolYearAction(name: string) {
  const session = await auth.api.getSession({ headers: await headers() });
  if (!session || session.user.role !== "admin") throw new Error("Forbidden");
...
export async function deleteSchoolYearAction(id: string) {
  ...
  await sql`DELETE FROM school_years WHERE id = ${id}`;
  revalidatePath("/admin/ppmp");
  return { error: null };
}
